...
export class Mineweb {
     _noaOpts: {
         debug: boolean; showFPS: boolean; chunkSize: number; chunkAddDistance: number; chunkRemoveDistance: number; tickRate: number; // ms per tick - not ticks per second
     };
     _moveOpts: {
         airJumps: number; maxSpeed: number; moveForce: number; standingFriction: number; // From Mineflayer - not sure if the units are the same
     };
    ...
    startNoa(noaOpts: Partial<{ debug: boolean; silent: boolean; playerHeight: number; playerWidth: number; playerStart: number[]; playerAutoStep: number; tickRate: number; blockTestDistance: number; stickyPointerLock: boolean; dragCameraOutsidePointerLock: boolean; skipDefaultHighlighting: boolean; originRebaseDistance: number; } & RenderingOptions>, moveOpts: Object) {
        const self = this;

...
import * as _mcData from "minecraft-data";
import { Scene } from '@babylonjs/core';
const mcData = _mcData("1.12.2");
...
function makeFlowerSpriteMesh(scene: Scene, url: string, name: string): Mesh {
    const matname = name || 'sprite-mat';
    const tex = new Texture(url, scene, true, true, Texture.NEAREST_SAMPLINGMODE);
    tex.hasAlpha = true;
    const mesh = Mesh.CreatePlane('sprite-' + matname, 1, scene);
    const mat = new StandardMaterial('sprite-mat-' + matname, scene);
...
export function registerTextures(noa: Engine): void {
    /* noa.registry.registerMaterial(
      "dirt",
      [Math.random(), Math.random(), Math.random()],
      "https://mineweb-dev.glitch.me/textures/dirt.png"
    ); // Random colours */
    noa.registry.registerMaterial("grass_top", [Math.random(), Math.random(), Math.random()], "https://raw.githubusercontent.com/MinewebMC/mineweb-assets/master/data/1.12/blocks/grass_top.png"); // Random colours
    noa.registry.registerMaterial("grass_side", [Math.random(), Math.random(), Math.random()], "https://raw.githubusercontent.com/MinewebMC/mineweb-assets/master/data/1.12/blocks/grass_side.png"); // Random colours
    noa.registry.registerMaterial("log_side", [Math.random(), Math.random(), Math.random()], "https://raw.githubusercontent.com/MinewebMC/mineweb-assets/master/data/1.12/blocks/log_oak.png");
    noa.registry.registerMaterial("log_top", [Math.random(), Math.random(), Math.random()], "https://raw.githubusercontent.com/MinewebMC/mineweb-assets/master/data/1.12/blocks/log_oak_top.png"); // Random colours
    ...
    for (let i = 1; i < 256; ++i) {
        try {
            textureURL = data.filter(function (block) {
        ...
        /* if (i == 3) {
          ...
          noa.registry.registerBlock(i, { material: "dirt" });
            ...
            const flowerMesh = makeFlowerSpriteMesh(scene, 'https://raw.githubusercontent.com/MinewebMC/mineweb-assets/master/data/1.12/blocks/tallgrass.png', 'flower');
            ...
            noa.registry.registerBlock(i, {
                ...
                material: 'flowerMat',
                onCustomMeshCreate: function (mesh: Mesh) {
                    mesh.rotation.y = Math.random() * Math.PI * 2;
                }
            });
        }
        else {
            noa.registry.registerMaterial(i.toString(), [Math.random(), Math.random(), Math.random()], 
            // "https://raw.githubusercontent.com/InventivetalentDev/minecraft-assets/1.12.2/assets/minecraft/textures/" +
            "https://raw.githubusercontent.com/MinewebMC/mineweb-assets/master/data/1.12/" +
                textureURL +
                ".png"); // Random colours

...
export class Mineweb {
    _noaOpts: {
        debug: boolean; showFPS: boolean; chunkSize: number; chunkAddDistance: number; chunkRemoveDistance: number; tickRate: number;
    };
    _moveOpts: {
        airJumps: number; maxSpeed: number; moveForce: number; standingFriction: number;
    };
    _clientOpts: mc.ClientOptions;
    plugins: unknown;
    _client: mc.Client;
    _noa: Engine;
    inventory: string;
    ...
    start(username: string): void {
        ...
        this._client.on("login", function () {
            ...
            this.registerPlugins(); // Use self since we're inside a new function
            this.startNoa(this._noaOpts, this._moveOpts);
    ...
    startNoa(noaOpts: mc.ClientOptions, moveOpts: Object): void {
        const self = this;
        // Engine options object, and engine instantiation:
        this._noa = new Engine(noaOpts);
        this._noa.ents.addComponentAgain(this._noa.playerEntity, this._noa.ents.names.movement, moveOpts);
        this._noa.world.manuallyControlChunkLoading = true;
        // setEventInputs(this._noa);
        ...
        // }
        this._noa.world.on("worldDataNeeded", function (id: number, data: number, x: number, y: number, z: number) {
                ...
                for (let i = 0; i < data.shape[0]; i++) {
                    for (let j = 0; j < data.shape[1]; j++) {
                        for (let k = 0; k < data.shape[2]; k++) {
                            const voxelID = chunksToLoad[`${x / 16}|${y / 16}|${z / 16}`].getBlock(new Vec3(k, y + j, i)).type; // x and z are reversed because otherwise it looks wrong
        ...
        const player = this._noa.playerEntity;
        const dat = this._noa.entities.getPositionData(player);
        const w = dat.width;
        const h = dat.height;
        const scene = this._noa.rendering.getScene();
        ...
        this._noa.on("tick", function (dt: any) {
            const playerPos = self._noa.ents.getPosition(self._noa.playerEntity);
            ...
            const scroll = self._noa.inputs.state.scrolly;
            if (scroll !== 0) {
                self._noa.camera.zoomDistance += scroll > 0 ? 1 : -1;
                if (self._noa.camera.zoomDistance < 0)
                    self._noa.camera.zoomDistance = 0;
                if (self._noa.camera.zoomDistance > 10)
                    self._noa.camera.zoomDistance = 10;
            }
        });
    }
}